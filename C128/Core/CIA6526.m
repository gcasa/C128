...
- (uint8_t)read:(uint8_t)r
{
  ...
  switch (r)
    ...
    case 13:
      {
        uint8_t value = pending | ([self irq] ? 128 : 0);
        pending = 0;
        return value;
      }
    default:
...
- (void)write:(uint8_t)v reg:(uint8_t)r
{
  ...
  switch (r)
    {
    case 4:
      latchA = (latchA & 0xff00) | v;
      break;
    case 5:
      latchA = (latchA & 255) | (v << 8);
      if (!(registers[14] & 1))
        timerA = latchA;
      break;
    case 6:
      latchB = (latchB & 0xff00) | v;
      break;
    case 7:
      latchB = (latchB & 255) | (v << 8);
      if (!(registers[15] & 1))
        timerB = latchB;
      break;
    case 12:
...
- (void)tick
{
  ...
  if (underflowA && (registers[14] & 64) && serialEdges && --serialEdges == 0)
    pending |= 8;
